...
public class SparseController : MonoBehaviour {
	...
	//public GameObject characterSphere;
	private int prefab_P = 0;

	void Start () {
		prefab_P = GameObject.Find ("Points").transform.childCount;
		//print ("prefab_P: " + prefab_P);
		//characterSphere = 
		//Transform beginPoint = GameObject.Find("CharacterSphere").transform;
		//Cpoints.Add (beginPoint);
		CreateNewRoad();
	...
	void CreateNewRoad()
	{
		...
		if(CurrentRoadId > 0)
		{
			...
			CurrentRoads.Add(Instantiate (Road, Cpoints[LevelCount * (prefab_P - 1) + LevelCount - 1].position, 
	...
	void Update () {
		/* ORIGINAL ASSUMES WE TRAVEL ON X
		...
		*/
		// We used Z
		Vector3 startPoint = new Vector3 (Cpoints [CurrentPoint].position.x, 
			Cpoints [CurrentPoint].position.y - 0.5f, 
			Cpoints [CurrentPoint].position.z + distance);
		Vector3 endPoint = new Vector3 (Cpoints [CurrentPoint + 1].position.x, 
			Cpoints [CurrentPoint + 1].position.y - 0.5f, 
			Cpoints [CurrentPoint + 1].position.z + distance);
		...
		if(time > 1)
		{
			...
			//CurrentPoint++;
			CurrentPoint = 0;
			string generatorName = "P" + (prefab_P - 3);
			if(Cpoints [CurrentPoint].name == generatorName)
			{
				CurrentRoadId++;
				LevelCount++;
				CreateNewRoad();
